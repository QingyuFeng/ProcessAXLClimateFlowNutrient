...
df_end_time = "2008-12-31 23:59:00"
...
def read_var(siteidx):
    ...
    if os.path.isfile(inf_lst[siteidx]):
        print("Solar Radiation data exists! Good!!!")
    else:
        print("Solar Radiation data not exists! Please check!!!")
...
def group_daily_subdaily(df_10min_filled_nt, df_10min):
    ...
    # Before the grouping happens, the solar radiation data
    # need unit conversion. 
    # Current unit is W/m2, and the required unit is MJ/m2.
    # 1 Kwh/m2/day = 3.6 Mj/m2/day
    # 1 W/m2 = 1W/m2 / 1000 * 10min/60min
    #        = 1/6000 Kwh/m2
    
    df_10min[infd_lst[0]] = df_10min.divide(6000, axis=[infd_lst[0]])
    df_10min_filled_nt[infd_lst[0]] = df_10min_filled_nt.divide(6000, axis=[infd_lst[0]])
    ...
    df_daily_var_ntf = \
        df_10min_filled_nt.groupby(pd.TimeGrouper(freq="1D")).sum()
    

    # Group monthly mean
    month_mean_ntf = df_10min_filled_nt.\
                groupby(pd.TimeGrouper(freq="1M")).sum()
...
def writing_outfiles():
    ...
    print("Writing daily average solar radiation with next day filled")
    ...
    print("Writing daily average solar radiation with no filled")
    ...
    print("Writing monthly mean and std of solar radiation with no filled")
    ...
    print("Writing monthly mean and std of solar radiation with next time filled")
...
print("Filling missing data for solar radiation in dataframe")
df_10min_filled_nt = fill_df_10min_nt(df_10min)
print("Finished filling missing data for solar radiation in dataframe")
#
print("Aggregating solar radiation data")
df_daily_var_nf, df_daily_var_ntf,\
df_month_var_avg_ntf,df_month_var_avg_nf,\
df_month_var_avg_std_ntf, df_month_var_avg_std_nf\
	= group_daily_subdaily(df_10min_filled_nt, df_10min)
print("Finished aggregating solar radiation data")
