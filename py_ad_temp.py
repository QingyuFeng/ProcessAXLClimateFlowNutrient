...
def read_var(siteidx):
    ...
    if os.path.isfile(inf_lst[siteidx]):
        print("Temperature data exists! Good!!!")
    else:
        print("Temperature data not exists! Please check!!!")
...
def fill_df_10min_nt(df_10min):
    ...
    # If there is no var data, the later one will be assigned
        if pd.isnull(df_10min_filled_nt[infd_lst[0]][df_10min_filled_nt.index[dffidx]]):
            print(df_10min_filled_nt[infd_lst[0]][df_10min_filled_nt.index[dffidx]])
            print(df_10min_filled_nt[infd_lst[0]][df_10min_filled_nt.index[dffidx+1]])
...
def group_daily_subdaily(df_10min_filled_nt, df_10min):
    ...
    df_daily_var_max_nf = \
        df_10min.groupby(pd.TimeGrouper(freq="1D")).max()
    df_daily_var_min_nf = \
        df_10min.groupby(pd.TimeGrouper(freq="1D")).min()

    # Daily maximum filled next day
    df_daily_var_max_ntf = \
        df_10min_filled_nt.groupby(pd.TimeGrouper(freq="1D")).max()
    df_daily_var_min_ntf = \
        df_10min_filled_nt.groupby(pd.TimeGrouper(freq="1D")).min()

    # Group monthly maximum and minimum mean
    month_sum_max_avg_ntf = \
        df_10min_filled_nt.groupby(pd.TimeGrouper(freq="1M")).max()
    df_month_var_max_avg_ntf = month_sum_max_avg_ntf.\
                    groupby(month_sum_max_avg_ntf.index.month).mean()
    
    month_sum_min_avg_ntf = \
        df_10min_filled_nt.groupby(pd.TimeGrouper(freq="1M")).min()
    df_month_var_min_avg_ntf = month_sum_min_avg_ntf.\
                    groupby(month_sum_min_avg_ntf.index.month).mean()
    
    month_sum_max_avg_nf = \
        df_10min.groupby(pd.TimeGrouper(freq="1M")).max()
    df_month_var_max_avg_nf = month_sum_max_avg_nf.\
                    groupby(month_sum_max_avg_nf.index.month).mean()
    
    month_sum_min_avg_nf = \
        df_10min.groupby(pd.TimeGrouper(freq="1M")).min()
    df_month_var_min_avg_nf = month_sum_min_avg_nf.\
                    groupby(month_sum_min_avg_nf.index.month).mean()
    
    # Group monthly maximum minimum std
    month_sum_max_std_ntf = \
        df_daily_var_max_ntf.groupby(pd.TimeGrouper(freq="1M")).std()
    df_month_var_max_std_ntf = month_sum_max_std_ntf.\
                    groupby(month_sum_max_std_ntf.index.month).mean()
    
    month_sum_min_std_ntf = \
        df_daily_var_min_ntf.groupby(pd.TimeGrouper(freq="1M")).std()
    df_month_var_min_std_ntf = month_sum_min_std_ntf.\
                    groupby(month_sum_min_std_ntf.index.month).mean()
    
    month_sum_max_std_nf = \
        df_daily_var_max_nf.groupby(pd.TimeGrouper(freq="1M")).std()
    df_month_var_max_std_nf = month_sum_max_std_nf.\
                    groupby(month_sum_max_std_nf.index.month).mean()
    
    month_sum_min_std_nf = \
        df_daily_var_min_nf.groupby(pd.TimeGrouper(freq="1M")).std()
    df_month_var_min_std_nf = month_sum_min_std_nf.\
                    groupby(month_sum_min_std_nf.index.month).mean()
    

    return df_daily_var_max_nf, df_daily_var_min_nf,\
	    df_daily_var_max_ntf, df_daily_var_min_ntf,\
            df_month_var_max_avg_ntf,  df_month_var_min_avg_ntf,\
		df_month_var_max_avg_nf, df_month_var_min_avg_nf,\
		 df_month_var_max_std_ntf,  df_month_var_min_std_ntf,\
		 df_month_var_max_std_nf,  df_month_var_min_std_nf

def writing_outfiles():
   
    print("Writing daily max and min temperature with next day filled")
    outfid_var_daily_ntf = open(outf_var_daily_ntf, "w") 
    for dldidx in df_daily_var_max_ntf.index:
        outfid_var_daily_ntf.writelines("%s\t%.2f\t%.2f\n" \
            %(dldidx, \
                float(df_daily_var_max_ntf[infd_lst[0]][dldidx]),\
                float(df_daily_var_min_ntf[infd_lst[0]][dldidx])\
                ))                
    outfid_var_daily_ntf.close()
 
    print("Writing daily max and min temperature with no filled")
    outfid_var_daily_nf = open(outf_var_daily_nf, "w")
    for dldidx in df_daily_var_max_nf.index:
        outfid_var_daily_nf.writelines("%s\t%.2f\t%.2f\n" \
            %(dldidx, \
                 float(df_daily_var_max_nf[infd_lst[0]][dldidx]),\
                 float(df_daily_var_min_nf[infd_lst[0]][dldidx])\
                ))
    outfid_var_daily_nf.close()

    print("Writing monthly mean and std of  max and min temperature with no filled")
    outfid_var_month_nf = open(outf_var_month_nf, "w")
    outfid_var_month_nf.writelines("Month\tAvgMax\tAvgMin\tSTDMax\tSTDMin\n")
    for dldidx in df_month_var_max_avg_nf.index:
        outfid_var_month_nf.writelines("%i\t%.2f\t%.2f\t%.2f\t%.2f\n" \
            %(dldidx,\
                float(df_month_var_max_avg_nf[infd_lst[0]][dldidx]),\
                float(df_month_var_min_avg_nf[infd_lst[0]][dldidx]),\
                float(df_month_var_max_std_nf[infd_lst[0]][dldidx]),\
                float(df_month_var_min_std_nf[infd_lst[0]][dldidx])\
                ))
    outfid_var_month_nf.close()

    print("Writing monthly mean and std of  max and min temperature with next day filled")
    outfid_var_month_ntf = open(outf_var_month_ntf, "w")
    outfid_var_month_ntf.writelines("Month\tAvgMax\tAvgMin\tSTDMax\tSTDMin\n")
    for dldidx in df_month_var_max_avg_ntf.index:
        outfid_var_month_ntf.writelines("%i\t%.2f\t%.2f\t%.2f\t%.2f\n" \
            %(dldidx,\
                float(df_month_var_max_avg_ntf[infd_lst[0]][dldidx]),\
                float(df_month_var_min_avg_ntf[infd_lst[0]][dldidx]),\
                float(df_month_var_max_std_ntf[infd_lst[0]][dldidx]),\
                float(df_month_var_min_std_ntf[infd_lst[0]][dldidx])\
                ))
...
print("Putting temperature data into timeframe")
df_10min = put_rainfall_to_dataframe(df_10min, lrf_var_all, infd_lst)
print("Finished putting temperature into timeframe")

print("Filling missing data fortemperature in dataframe")
df_10min_filled_nt = fill_df_10min_nt(df_10min)
print("Finished filling missing data for temperature in dataframe")
##
print("Aggregating flow and concentration data")
df_daily_var_max_nf, df_daily_var_min_nf,\
df_daily_var_max_ntf, df_daily_var_min_ntf,\
df_month_var_max_avg_ntf,  df_month_var_min_avg_ntf,\
df_month_var_max_avg_nf, df_month_var_min_avg_nf,\
df_month_var_max_std_ntf,  df_month_var_min_std_ntf,\
...
print("Finished aggregating temperature data")
